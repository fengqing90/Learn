...
public class ReportController {
     ...
     * 评审员服务
     */
    @Reference(version = "1.0.0", interfaceClass = ReviewerService.class,
            cluster = "failfast")
     ...
     * 奖励服务
     */
    @Reference(version = "1.0.0", interfaceClass = RewardService.class,
            cluster = "failfast")
    ...
    public String report(ReportTask reportTask) {
        // 在本地数据库增加一个举报任务
        this.reportTaskService.add(reportTask);

        // 调用评审员服务，选择一批评审员
        List<Long> reviewerIds = this.reviewerService
            .selectReviewers(reportTask.getId());
        // 在本地数据库初始化这批评审员对举报任务的投票状态
        this.reportTaskVoteService.initVotes(reviewerIds, reportTask.getId());
    ...
    public String vote(Long reviewerId, Long reportTaskId, Integer voteResult) {
        // 本地数据库记录投票
        this.reportTaskVoteService.vote(reviewerId, reportTaskId, voteResult);
        // 调用评审员服务，标记本次投票结束
        this.reviewerService.finishVote(reviewerId, reportTaskId);

        // 对举报任务进行归票
        Boolean hasFinishedVote = this.reportTaskVoteService
            .calculateVotes(reportTaskId);

        // 如果举报任务得到归票结果
        if (hasFinishedVote) {
            // 发放奖励
            List<ReportTaskVote> reportTaskVotes = this.reportTaskVoteService
            ...
            this.rewardService.giveReward(reviewerIds);
