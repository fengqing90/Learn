 ...
 * @describle 时期时间工具类
 * @author AlfredXiong
 * @time 2013-4-18 下午2:21:46
 */
public class DateTimeUtils {

    Log log = LogFactory.getLog(DateTimeUtils.class);
    ...
    public final static SimpleDateFormat stampDateFormat = new SimpleDateFormat(
        "HH:mm");
    ...
    public static int getNextDayOfweek() {
        ...
        calendar.setTime(new java.util.Date(new Date().getTime() + 24 * 60 * 60
            * 1000));
    ...
    public static String validateSimpleDate(Date date) {
        return date == null ? null : DateTimeUtils.simpleDateFormat
            .format(date);
    }
     ...
     * @return
     */
    public static String validateTimeStamp(Timestamp date) {
        return date == null ? "　　" : DateTimeUtils.stampDateFormat.format(date);
    }
    ...
    public static String validateDateTimeHM(Date date) {
        return date == null ? null : DateTimeUtils.dateTimeHMFormat
            .format(date);
    }
    ...
    /**
     * 计算两个时间之间的分钟数
     * 如果start在end之后，则返回start减去end的差值
     * 
     * @param start
     * @param end
     * @return
    ...
    /**
     * 根据分钟计算相应的小时数，用于乘法或者除法计算。
     * 每60分钟为一个小时，30分钟为0.5个小时
