...
public class Ali extends LeetCode {
     ...
     */
    final static class 背包问题_变种_2人分最小差值 extends Ali {
        public static void main(String[] args) {
            int[] arr = { 1, 2, 3, 4, 5 };
            int value = new 背包问题_变种_2人分最小差值().match(arr);
            System.out.println(value);
        }

        public int match(int[] arr) {
            int sum = Arrays.stream(arr).sum();
            int len = arr.length;
            int halfOfSum = sum / 2;
            // 确定矩阵二维定义：
            // 第一维代表前i个物体，i可为0；
            // 第二维代表从0开始的连续容量值确定矩阵长宽，并初始化。
            // 因为矩阵第一维和第二维都是从0开始，所以要加一
            ...
            for (int i = 1; i < matrix_firstDimensionLen; i++) {
                for (int j = 1; j < matrix_secondDimensionLen; j++) {
                    ...
                    int arr_i = arr[i - 1];
                    if (j - arr_i >= 0
                        && matrix[i - 1][j - arr_i] + arr_i > matrix[i][j]) {
                        matrix[i][j] = matrix[i - 1][j - arr_i] + arr_i;
                    }
                }
            }
            for (int[] ints : matrix) {
